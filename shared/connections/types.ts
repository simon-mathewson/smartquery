...
export const remoteEngineSchema = z.union([z.literal('mysql'), z.literal('postgres')]);
...
export const connectionSchema = z
  ...
  .refine(
    (conn) =>
      conn.type !== 'remote' || conn.credentialStorage !== 'encrypted' || Boolean(conn.password),
    {
      message: 'Password is required when credential storage is set to encrypted',
    },
  )
